...
export default function DashboardLayout({ children, params }: { children: ReactNode; params: { [key: string]: string | string[] } }) {
  // Compute pathname from params (for /dashboard and /dashboard/user-cabinet)
  let pathname = "/dashboard";
  if (Array.isArray(params?.slug)) {
    pathname = "/dashboard/" + params.slug.join("/");
  } else if (typeof params?.slug === "string") {
    pathname = "/dashboard/" + params.slug;
  }
    ...
    <div className="h-screen flex flex-col bg-gray-50">
      ...
      <header className="sticky top-0 z-20 w-full bg-white shadow px-4 py-2 flex flex-col gap-2 items-stretch">
        ...
        <div className="md:hidden">
          <NavMenuMobile currentPath={pathname} />
        </div>
      </header>
